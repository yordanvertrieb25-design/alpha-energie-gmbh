...
- Name: seo.spec.js >> SEO E2E Test Suite >> Tier 2: Boundary & Corner Cases >> 40. Title tags do not have duplicate contents across indexable pages
- Location: tests\seo.spec.js:484:5
...
> 487 |         await page.goto(`/${pageName}`);
  ...
  575 |         try {
  576 |           expect(sitemapUrls).not.toContain(file);
  577 |           
  578 |           await page.goto(`/${file}`);
  579 |           const metas = await getRobotsMetaContents(page);
  580 |           expect(metas.length).toBeGreaterThan(0);
  581 |           const hasNoIndexOrNone = metas.some(meta => meta.content.includes('noindex') || meta.content.includes('none'));
  582 |           expect(hasNoIndexOrNone).toBe(true);
  583 |         } catch (err) {
  584 |           errors.push({ file, error: err.message });
  585 |         }
  586 |       }
  587 |       expect(errors).toEqual([]);
```